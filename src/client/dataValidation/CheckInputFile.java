...
public class CheckInputFile implements InputChecker {
	...
	public Map<String, Object> checkInput() {
		...
		try {
			...
			if(CheckData.isInteger(dataMovie[6]) && Integer.parseInt(dataMovie[6])<=Client.genreNames.length){
				data.put("Genre", Client.genreNames[Integer.parseInt(dataMovie[6])-1]);
			}
			else if(dataMovie[6] == null || dataMovie[6].isEmpty()) {
				data.put("Genre", null);
			}
			else {
				if(Arrays.asList(Client.genreNames).contains(dataMovie[6].toUpperCase())) {
			...
			if(CheckData.isInteger(dataMovie[9]) && Integer.parseInt(dataMovie[9])<=Client.colorNames.length){
				data.put("Operator_Eye", Client.colorNames[Integer.parseInt(dataMovie[9])-1]);
			}
			else if(dataMovie[9] == null || dataMovie[9].isEmpty()) {
				data.put("Operator_Eye", null);
			}
			else {
				if(Arrays.asList(Client.colorNames).contains(dataMovie[9].toUpperCase())) {
			...
			if(CheckData.isInteger(dataMovie[10]) && Integer.parseInt(dataMovie[10])<=Client.colorNames.length){
				data.put("Operator_Hair", Client.colorNames[Integer.parseInt(dataMovie[10])-1]);
			}
			else if(dataMovie[9] == null || dataMovie[10].isEmpty()) {
				data.put("Operator_Hair", null);
			}
			else {
				if(Arrays.asList(Client.colorNames).contains(dataMovie[10].toUpperCase())) {
			...
			if(CheckData.isInteger(dataMovie[11]) && Integer.parseInt(dataMovie[11])<=Client.countryNames.length){
				data.put("Operator_Nation", Client.countryNames[Integer.parseInt(dataMovie[11])-1]);
			}
			else if(dataMovie[11] == null || dataMovie[11].isEmpty()) {
				data.put("Operator_Nation", null);
			}
			else {
				if(Arrays.asList(Client.countryNames).contains(dataMovie[11].toUpperCase())) {
